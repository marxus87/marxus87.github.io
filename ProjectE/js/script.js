...
var skills = [];
skills = _.map(objectJSON, 'skills');
skills = _.flattenDeep(skills);
skills = _.intersection(skills);
skills = _.orderBy(skills, [], 'asc');
var friends = _.map(objectJSON, 'friends');
friends = _.flatten(friends);
friends = _.map(friends, 'name');
friends = _.intersection(friends);
var names = objectJSON;
for (i=0; i<objectJSON.length; i++) {
  names[i].friends = objectJSON[i].friends.length
}
